---
layout: farm
title: Ramharter Biodynamic Farm
slug: ramharter-biodynamic-farm-canterbury
name: Ramharter Biodynamic Farm
type: farm
subtype: biodynamic-market-garden
country: NZ
country_slug: new-zealand
region: Canterbury
city: Ladbrooks (near Christchurch)
address: Ladbrooks, Canterbury, New Zealand
postcode: ""
lat: -43.615853860001685
lon: 172.53194927316287
description: >
  A Canterbury-based biodynamic market garden producing seasonal vegetables using holistic, beyond-organic farming methods focused on soil health, lunar rhythms, and ecological balance.
long_description: >
  Ramharter Biodynamic Farm is a small-scale market garden located in Ladbrooks, Canterbury, near Christchurch, and is closely associated with grower Rainer Ramharter. The farm is known locally for producing high-quality seasonal vegetables using biodynamic principles—an approach to agriculture that treats the farm as a self-sustaining living organism.
  
  Farming practices at Ramharter go beyond standard organic methods. Synthetic fertilisers, herbicides, and pesticides are avoided entirely, with soil fertility instead built through composting, crop rotation, cover cropping, and biodynamic preparations. Planting and harvesting are informed by lunar and seasonal calendars, a core feature of biodynamic agriculture, with the intention of strengthening soil life, plant vitality, and overall ecosystem health.
  
  Produce from the farm is sold directly into the local food system, including through farmers’ markets such as the Mount Pleasant Farmers’ Market and via community-focused retailers like Harbour Co-op in Lyttelton. The emphasis is on freshness, locality, and transparency, with vegetables grown specifically for the Canterbury region rather than long-distance distribution.
  
  While formal biodynamic certification (such as Demeter) is not publicly confirmed, the farm’s practices align closely with biodynamic and regenerative principles. Ramharter Biodynamic Farm is regarded as part of Christchurch’s grassroots organic food network, contributing to resilient local supply chains and healthier soils.
practices:
  - biodynamic
  - organic-principles
  - spray-free
  - regenerative-soil-building
products:
  - seasonal-vegetables
services:
  - direct-market-sales
  - farmers-market-sales
website: ""
email: ""
phone: ""
social_links: []
hours: Seasonal; availability varies by market and harvest
market_days:
  - Mount Pleasant Farmers Market
certifications: []
awards: []
supply_role: Producer
source_urls: []
sources: []
supplies_to:
  - harbour-co-op-lyttelton
also_referenced_by: []
last_checked: "2025-12-15"
practices_tags:
  - biodynamic
  - regenerative
  - organic
products_tags:
  - vegetables
services_tags:
  - farmers-market
specialty_tags:
  - local-producer
  - beyond-organic
relationships_declared: true
---
