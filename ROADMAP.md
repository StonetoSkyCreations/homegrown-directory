...
- **Harvest pipeline backbone (2026-06-28):** credit-light, evidence-first discovery pipeline (full doc in `HARVEST.md`). `scripts/hub_report.rb` (ranks NZ hubs), `scripts/harvest_lib.rb` (listing index + matcher), `scripts/match_report.rb` (dupe detector), `scripts/harvest_import.rb` (bulk wrapper over `add_listing.rb` + evidence ledger + reciprocate), `scripts/evidence_audit.rb`, and the `_data/relationship_evidence.yml` edge-evidence ledger. Relationships stay plain slug arrays, so maps/search/audit are unaffected.
- **Harvest Pilot 1, AsureQuality register (2026-06-28):** `scripts/harvesters/asurequality.py` parses the certified-organic register to `data/imports/asurequality.csv` (206 deduped NZ operators). Used as a curation source (no websites, legal names): enriched 7 confirmed existing listings with `AsureQuality Organic` certification via `scripts/enrich_certifications.rb`. Remaining ambiguous matches + new producers staged for review in `SOURCING_PLAN.md`.
- **Harvest Pilot 4, producer-forward KML apply (2026-06-29):** built the scan -> extract -> classify chain (`scripts/harvesters/stockist_scan.py`, `stockist_extract.py`, `scripts/candidate_edges.rb`) and applied the two clean KML hubs CoralTree + Three Oaks: 17 AUTO edges to existing stockists + 11 curated new outlets, 28 reciprocal relationships, pairs 240 -> 268, all evidenced. Curation dropped clinics/pharmacies, 3 producers mis-listed as stockists, and a "coming soon" pin. Next leverage is per-widget adapters; resume block + deferred items in `SOURCING_PLAN.md`.
...
## Next
1. **NZ sourcing: the connection harvest** — the main growth workstream. Mine "hubs" (a producer's stockist page, or a stockist/distributor's supplier page) to add many listings + reciprocal relationships per pass, NZ only for now. Current priority: keep harvesting cafes, restaurants, grocers, markets and distributors through verified producer links. Tooling and the two clean KML hubs (CoralTree, Three Oaks) are done; the next technical step is per-widget stockist adapters (WP Store Locator first, then Wix, then Shopify) to unlock the producer pages locked behind JS. Full method, the **resume block**, and a per-session checklist live in **`SOURCING_PLAN.md`**. One hub ≈ one session ≈ one commit, so credits stay bounded.
